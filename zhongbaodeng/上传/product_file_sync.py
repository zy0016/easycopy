...
class SSHSession(object):
    ...
    def getDurationFundinfo(self,startDate,endDate,filelocalpath):
        ...
        sql="select  t.l_id 序号,  a.c_fundcode   产品代码, a.c_tafundcode TA产品代码, a.c_fundname 产品名称, decode( a.L_PDTTEMPLETID,'6','债权投资计划','8','资产支持计划') 产品类型," \
            " decode( t.c_type,'0','收益分配','1','赎回','2','回售','3','分期还本','4','兑付','5','清算','6','分红') 业务类型,  " \
            " (CASE c_type WHEN '0' THEN  c_date WHEN '1' THEN  c_factdate WHEN '2' THEN  c_factdate WHEN '3' THEN  c_factdate WHEN '4' THEN  c_factdate ELSE  c_date END) 业务发生日期 ," \
            " f_totalamount 业务发生总额, s_severaltime 期次," \
