...
class SSHSession(object):
    ...
    def read_tblobstorage_table_attachmentCount(self,dest_date):
        ...
        savelog("read_tblobstorage_table_attachmentCount " + sql_fileid)
        dataresult = self.cursor.execute(sql_fileid)
        data = dataresult.fetchall()
        return int(data[0][0])
    
    def read_tblobstorage_debug(self):
        sql_fileid = "select count(*) from interins.sys_dictionary"
        dataresult = self.cursor.execute(sql_fileid)
        data = dataresult.fetchall()
        print(data)
        return int(data[0][0])
    ...
    def read_tblobstorage_table_attachment(self,fileremotepath,filelocalpath,startDate,dest_date,pagecount):
        ...
        savelog("read_tblobstorage_table_attachmentCount return:"+str(amount))
        returnset = set()
        page_number = 0
