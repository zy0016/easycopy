...
class FTPSync(object):
    ...
    def upload_dir(self, local_path, ftp_path='.', begin=True):
        remote_sep = "/"
        tmpftp_path = ftp_path.rstrip('/')
        rebak = self.conn.pwd()
        # 当本地目录存在时上传
        if os.path.isdir(local_path):
            ...
            if begin:
                if not self._is_ftp_dir(tmpftp_path):
                    ...
                    for i in dirarr:
                        p1 = str(i)
                        p2 = p1.strip()
                        checkpath = checkpath + p2 + remote_sep
                        if len(p2) > 0 and os.path.exists(checkpath) == False:
                            try:
                                self.conn.mkd(p2)
            ...
            for file in local_files:
                ...
                if os.path.isdir(file):
                    ftp_path = os.path.join(tmpftp_path, file)
                    self.put_dir(file, tmpftp_path+remote_sep+str(file), False)
                else:
                    source_file = local_path + remote_sep + file
                    if self.UTF8_PATH:
                        dest_file = "." + tmpftp_path + remote_sep + file
                    else:
                        extendid = file.rindex('.')
                        filenewname = ""
                        if extendid == -1:
                            filenewname = str(i)
                        else:
                            filenewname = str(i) + file[extendid:]

                        dest_file = "." + tmpftp_path + remote_sep + filenewname

                    dest_file = dest_file.replace("\\","/")
                    filelist.append(source_file + "," + dest_file)
        ...
        for id in filelist:
            idarr = id.split(',')
            self.uploadfile(idarr[0],idarr[1])
    ...
    def uploadfile(self,sourcefile,desfile):
        self.savelog("upload file " + sourcefile + " to " + desfile)
        file_handler = open(sourcefile, "rb")
        self.conn.storbinary("STOR " + desfile, file_handler, 1024)
        file_handler.close()

if __name__ == '__main__':
    ftp = FTPSync('192.168.10.96')
    ftp.login('root', 'P@ssw0rd')
    # 上传文件，不重命名
